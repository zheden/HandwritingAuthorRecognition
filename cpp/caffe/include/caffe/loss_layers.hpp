 ...
 *      the features @f$ a \in [-\infty, +\infty]@f$
 *   -# @f$ (N \times C \times 1 \times 1) @f$
 *      the similar features @f$ b \in [-\infty, +\infty]@f$
 *   -# @f$ (N \times 1 \times 1 \times 1) @f$
 *      the dissimilar features @f$ c \in [-\infty, +\infty]@f$
 * @param top output Blob vector (length 1)
 * This can be used to train triamese networks.
 */
template <typename Dtype>
class TripletLossLayer : public LossLayer<Dtype> {
 public:
  explicit TripletLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_same_(), diff_other_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
 ...
 protected:
   ...
   *
   * Computes the gradients with respect to the three input vectors (bottom[0] and
   * bottom[1], bottom[2]).
   *
   * @param top output Blob vector (length 1), providing the error gradient with
   *      respect to the outputs
   ...
   *      propagate_down[1]
   *   -# @f$ (N \times C \times 1 \times 1) @f$
   *      the features @f$c@f$; Backward fills their diff with gradients if
   *      propagate_down[2]
   */
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  ...
  Blob<Dtype> diff_same_;  // cached for backward pass
  Blob<Dtype> diff_other_;  // cached for backward pass
