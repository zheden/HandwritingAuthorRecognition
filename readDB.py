...
def padWithOnesOrCut(line, i_maxHeight):
    ...
    curWidth = line.shape[1]
    if MAX_WIDTH > curWidth:
        line = np.pad(line, ((0, 0), (0, MAX_WIDTH - curWidth)), mode='constant', constant_values=(255))[:, :]
    else:
...
#####################################################################
def getLMDBEntryTriplet(i_image1, i_image2, i_image3, i_label):
    datum = caffe.proto.caffe_pb2.Datum()
...
MAX_LINES_TO_TRAIN = 10 # NOTE: this is number without permutations
MAX_LINES_TO_TEST = 5
MAX_NUM_WORDS_PERMUTATIONS = 3
...
# preprocess images
print '------ Train set:'
(linesToTrain, lineHeight) = preprocessImages(linesToTrain)
print '------ Test set:'
(linesToTest, lineHeight)  = preprocessImages(linesToTest, lineHeight)
...
scipy.misc.imsave('testPair_from_trainLMDB1.jpg', im[1, :, :]) # im in second channel
scipy.misc.imsave('testPair_from_trainLMDB0.jpg', im[0, :, :])
if (packToTriplets):
    scipy.misc.imsave('testPair_from_trainLMDB2.jpg', im[2, :, :])
